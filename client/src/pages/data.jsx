...
// const data = () => {
//   const [isLoggedIn, setIsLoggedIn] = useState(true); // Example state for logged-in status

//   const handleLogout = () => {
//     // Implement logout logic here (clear session, state, etc.)
//     setIsLoggedIn(false); // Example: Set isLoggedIn to false after logout
//   };

//   return (
//     <div>
//       {/* Render CustomNavbar component and pass props */}
//       <CustomNavbar isLoggedIn={isLoggedIn} handleLogout={handleLogout} />
//       {/* Other content of the logged-in page */}
//     </div>
//   );
// };
        ...
        <thead>
          ...
          {users.map((user) => (
            <tr key={user.empID} >
              ...
              <td>{user.password}</td>
              <td>{user.role}</td>
              {location.state && location.state.user.role==='admin'? <td><Button variant="danger" onClick={()=>handledeleteuser(user.id)}>Delete</Button></td>:<div></div>}
            </tr>
          ))}
          </tbody>
        </Table>
